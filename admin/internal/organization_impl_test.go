...
func Test_internalOrganizationImpl_GetGroupsV2(t *testing.T) {
	...
	}{
		{
			...
			on: func(fields *fields) {
				...
				client.On("NewRequest",
					context.Background(),
					http.MethodGet,
					"admin/v2/orgs/organization-sample-uuid/directories/directory-sample-uuid/groups?cursor=cursor-sample-uuid&limit=20",
					"",
					nil).
					Return(&http.Request{}, nil)
				...
				client.On("Call",
					&http.Request{},
					&model.OrganizationGroupsV2Page{}).
					Return(&model.ResponseScheme{}, nil)
	...
	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			...
			gotResult, gotResponse, err := newOrganizationService.GetGroupsV2(testCase.args.ctx, testCase.args.organizationID, testCase.args.directoryID, testCase.args.params)
